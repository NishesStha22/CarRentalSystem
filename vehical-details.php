        ...
        <div class="main_features">
          <ul>
            ...
            function hasfav($a,$b){
                ...
                $sql = "SELECT * FROM `fav` WHERE car='$a' AND email = '$b' ";
                $res = $con->query($sql);
                if($res->num_rows>0){
                    return 'fav-added';
                }else{
            ...
            ?>
            <script src="dist/jquery.star-rating-svg.min.js"></script>
...
<?php if(isset($_SESSION['login'])){?>
          <form action="ratings.php" method="post">
              ...
              <li>    
              <input type="hidden" name="vehicle_id" value="<?php echo $vhid?>">
            <input type="hidden" name="username" value="<?php echo $_SESSION['user']?>">          
                <select name="rating" id="rating">
                  <option value="0">--Rate this vehicle--</option>
                ...
                <button id="submit_rating" name="submit_rating">Rate</button>
              </li>
              </form>
              <?php }else{?><li>Please Login to rate</li><?php }?>
        ...
        <div class="sidebar_widget">
          ...
          <form method="post">
            <div class="form-group">
              ...
              <input type="date" id='fromdate' class="form-control" name="fromdate" placeholder="From Date" required>
            </div>
            <div class="form-group">
              <label>To Date:</label>
              <input type="date" id='todate' class="form-control" name="todate" placeholder="To Date" required>
            </div>
            <div class="form-group">
...
//$re = new Recommend();
// print_r($re->getRecommendations($books, "Test"));
//$array=$re->getRecommendations($cars, $_SESSION['login']);

//print email from session
//echo $_SESSION['login'];
...
// $bid=$_SESSION['brndid'];  
$bid=$vehicleName;
$sql="SELECT tblvehicles.VehiclesTitle,tblbrands.BrandName,tblvehicles.PricePerDay,tblvehicles.FuelType,tblvehicles.ModelYear,tblvehicles.id,tblvehicles.SeatingCapacity,tblvehicles.VehiclesOverview,tblvehicles.Vimage1 from tblvehicles join tblbrands on tblbrands.id=tblvehicles.VehiclesBrand join tbl_rating on tbl_rating.vehicle_id=tblvehicles.id ORDER BY tbl_rating.rating DESC";
...
<script>
  function favroite(id, email){

    // 
    if($('#favcarbtn').hasClass('fav-added')){
      $.ajax({
        method: "POST",
        url: "fav.php",
        data: { id: id , email:email, 'rem_fav': "addit" }
      })
        .done(function( msg ) {
          const result = msg.trim();
          if(result == 'sucess'){
            $('#favcarbtn').removeClass('fav-added');
          }else{
            alert(msg);
          }
        });
      ...
      $.ajax({
        method: "POST",
        url: "fav.php",
        data: { id: id ,email:email, 'add_fav': "addit" }
      })
        .done(function( msg ) {
          const result = msg.trim();
          if(result == 'sucess'){
               $('#favcarbtn').addClass('fav-added');
          }else{
            alert(msg);
